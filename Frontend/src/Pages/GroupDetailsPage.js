...
const GroupDetailsPage = () => {
  ...
  useEffect(() => {
    const init = async () => {
        try {
            const res = await api.get(`/${groupName}/group-details`, { headers : { 'username' : username }})
  ...
  return (
    <div className={`grid grid-rows-[64px_1fr] grid-cols-[64px_1fr] 
      ...
      <div className="w-full h-full bg-gray-900 overflow-y-auto p-8 text-gray-200 scrollbar-hide">
        <div className="max-w-4xl mx-auto">
          <div className="bg-gray-800 rounded-xl p-6 shadow-lg mb-6">
            <h2 className="text-2xl font-bold mb-4 text-indigo-400">Group Description</h2>
          ...
          <div className="bg-gray-800 rounded-xl p-6 shadow-lg">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold text-indigo-400">Group Members</h2>
              <button 
              ...
              >
                <UserPlus className="w-5 h-5" /> {showAddMembers ? 'Hide Form' : 'Add Members'}
              </button>
            </div>

            {showAddMembers && (
              <div className="mb-6 bg-gray-700 rounded-xl p-6">
                <h3 className="text-xl font-bold text-indigo-400 mb-4">Add New Members</h3>
